...
const getStatusLabel = (status: string): string => {
    switch (status) {
        ...
        case 'arrived_at_pickup': return 'Na Loja';
        case 'ready_for_pickup': return 'Pedido Pronto';
        case 'picked_up': return 'Coletado';
        case 'in_transit': return 'Em Rota';
        case 'arrived_at_customer': return 'No Cliente';
...
const getStatusBadgeClass = (status: string): string => {
    switch (status) {
        case 'pending':
            return 'bg-orange-500/10 text-orange-400 animate-pulse border-orange-500/20';
        case 'ready_for_pickup':
            return 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20 shadow-glow-cyan';
        case 'in_transit':
        case 'picked_up':
...
const getStatusDescription = (status: string): string => {
    switch (status) {
        case 'pending': return 'Procurando pilotos...';
        case 'accepted': return 'Piloto aceitou a corrida...';
        case 'arrived_at_pickup': return 'Piloto chegou na loja...';
        case 'ready_for_pickup': return 'Pedido pronto na loja...';
                ...
                {activeDeliveries.length === 0 ? (
                    ...
                    activeDeliveries.map((delivery) => {
                        ...
                        return (
                            ...
                            >
                                {/* Header row: ID + source tags + status badge */}
                                ...
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-xs font-black text-white truncate">
                                            {delivery.status === 'pending' ? 'Alocando...' : (delivery.driver_name || 'Alocando...')}
                                        </p>
                                ...
                                <div className="relative flex items-center justify-between mb-4 px-1 select-none">
                                    {/* Background track */}
                                    <div className="absolute top-[7px] left-0 w-full h-0.5 bg-white/5 rounded-full -z-0"></div>
                                    {/* Filled track */}
                                    <div
                                        className="absolute top-[7px] left-0 h-0.5 bg-guepardo-orange rounded-full -z-0 transition-all duration-1000 shadow-[0_0_8px_rgba(255,107,0,0.5)]"
                                    ...
                                    {STEPS.map((step, index) => {
                                        ...
                                        return (
                                            <div key={step.label} className="relative z-10 flex flex-col items-center">
                                                <div className={cn(
                                                    "w-3.5 h-3.5 rounded flex items-center justify-center border transition-all duration-500",
                                                    isCompleted
                                                        ? "bg-guepardo-orange border-guepardo-orange text-white shadow-glow"
                                                        : "bg-[#1C1917]/80 border-white/5 text-white/10",
                                                    isCurrent && "ring-2 ring-guepardo-orange/30 animate-pulse scale-110"
                                                )}>
                                                    {isCompleted ? (
                                                ...
                                                <span className={cn(
                                                    ...
                                                    isCurrent ? "text-guepardo-orange" : "text-white/20"
                                                )}>
                                                    {step.label}
                                                </span>
                                            </div>
